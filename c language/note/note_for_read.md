...
运算优先级 
![运算优先级](photos_for_note/photo_1.png)
...
![补码](photos_for_note/photo_2.png)
...
<br>
我们也可以用科学计数法来表达数
![科学计数法](photos_for_note/photo_3.png)
...
![逃逸字符](photos_for_note/photo_4.png)
...
接下来我们都把a当作一个数组来看
<br>
`int a[100]`定义一个长度为100的int类型数组，即这个数组里的每个元素都是int类型
<br>
`int a[] = {2,4,6,7,1,3,5,9,11,13,23,14,32};`定义一个数组还可以采用数组的集成初始化
<br>
集成初始化如果规定了大小并且给的元素数量小于数组长度时，那么其余的位置均会变成零，如`int a[5]={2,6}`实际上是`a[5]={2,6,0,0,0}`
<br>
在c99后，还可以这样，`int a[5] = {[0] = 2,[2] = 3,6,};`实际上等于`a[5]={2,0,3,6,0}`
<br>
我们`sizeof(a)`去查看数组时，是得到整个数组占的内存,如果拿这个内存除以数组类型或者除以`sizeof(a[0])`就可以得到数组长度大小
<br>
但是在函数中传入数组时，如int max(int a[]),我们不能传入数组的大小，只能传入这个数组的内存地址，同时因为传入的是地址，我们也无法在函数内用sizeof计算数组的大小,计算出来只会得到a类型的内存大小
<br>
c99前数组的大小是固定的，即定义其长度时只能用确定的数字而不可以用变量，但是c99后就可以了
<br>
数组的元素在内存中是连续排列的
<br>
数组是从0开始计数的
<br>
当对于一个固定大小的数组，如果存储的元素多于这个数组，就会越界，是非常危险的行为
<br>
除了一维数组，我们还可以有二维数组甚至多维数组,直接用int a[3][5]可以定义出一个三行五列的数组,对于二维数组，列数是一定要规定的，行数可以省略，对于多维数组也可以类比
...
输出地址用%p
<br>
数组的名字a就是a[0]的地址，也就是数组的首地址
<br>
指针(point)变量就是用来保存地址的变量,

```c
int i;
int* p=&i;//这个*号可以远离变量，即这里所展示的，也可以靠近变量，如int *p;两者是等价的
//如果是int* p, q;那么p是一个指针，而q只是一个普普通通的int类型变量
```

`void max(int *p)`这样指针就会作为一个函数的形参,然后在调用这个函数的时候，就需要这样交给这个函数一个地址`max(&a)`
<br>
这样我们就可以在函数内部通过地址访问i的值,没有指针我们是做不到这件事的
<br>
*还可以作为解地址的运算符,即*p就是p所指向地址上的值,往往对指针的疑惑就是在这个*到底是理解为运算符还是理解为定义指针上，我们可以直接*p=6这样去修改这个地址上的值,swap函数就是这样的应用
...
在一个函数中往往只能返回一个值或者零个值，但我们可以用指针来返回多个值，即无需return，直接将值写到地址里
<br>
定义完指针后，需要让它指向一个地址，然后才能使用，不然会变成野指针，这个是非常危险的行为,我们往往可以先让这个指针指向空，即指向null
<br>
实际上数组变量是特殊的指针，函数中传入数组实际上就是传入指针，我们无需对数组取地址就可以写出`int *p=a`，同样在函数里修改数组的值也同样会影响到函数外中这个数组的值
<br>
...
<br>
指针计算往往会用到加减整数，递增递减，以及两个指针相减,还有内存地址大小比较,*的运算优先级没有递增和递减高
<br>
内存中会存在0地址，但是这个地址不能做任何事情，所以我们可以初始化指针时可以让指针先指向0，即指向NULL
<br>
指向不同类型的指针不可以互相赋值;void*表示指向的地址类型不确定,计算时与char相同;指针也可以进行强制类型转换
<br>
c99之前，无法用变量来定义数组的大小，想要得到大小可变的数组就要用到动态内存分配，同时要加一个头文件
<br>
`malloc`代表申请内存，但是申请来的内存我们需要给它一个类型，于是用到强制类型转换`(int*)`，然后申请大小为int乘以变量大小的内存，这个时候要用到`sizeof()`，然后a就可以当作数组来使用了，使用完后我们要将内存还回去，即`free(a)`，否则内存会泄露
...
```
当内存不够用时，申请的内存会返回0或者null
<br>
指针作为一个类型，我们当然可以有指针类型数组，这样我们就可以malloc多维数组,下面就malloc了一个n*4的数组
